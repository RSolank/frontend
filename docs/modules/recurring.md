...
| File                                  | Purpose                                                                                                                                                                                                                                                                          |
| ------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `pages/RecurringPage.tsx`             | Three-tab management surface (B3). **Detected** = `candidate` only (confirm/dismiss CTAs; capped to a 5-row preview with a "show more" reveal). **Confirmed** = everything the user owns — `review` (heading toned amber, "Needs attention"), `locked` ("Confirmed"), and paused `inactive` ("Paused"). **Upcoming** = the 30-day forecast list. Default tab is Detected when candidates await a decision, else Confirmed. A `?template=<uid>` deep-link (from a transaction's recurring chip) selects the tab the target lives in, expands the Detected reveal if hidden, then flashes + scrolls the row. "+ Add manually" in the header opens `RecurringFormDialog`. |
...
| `pages/RecurringPage.test.tsx`            | Empty state; default-to-Detected + status→tab routing (Confirmed holds review→locked→paused in order); Confirm shown on detected / hidden on confirmed-locked; Detected "show more" reveals beyond the preview; `?template` deep-link lands on the correct tab (candidate vs confirmed); tab toggle swaps to upcoming-empty. |
