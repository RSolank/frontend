...
- **TransactionsPage** — table scrolls _inside_ its card
...
| Motion scaffold (`T-motion-scaffold` — app-wide motion + dashboard + landing)                                                                          | **124.56 kB** | **+~9 kB**  | `MotionProvider` (LazyMotion + framer core) hoisted app-wide; framer **features stayed lazy**. JS ceiling 125 → 135, CSS 15 → 16 (temp headroom for the motion rollout).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| Landing refresh (`T-landing-content-refresh` — savings-led framer hero + reworked showcase strip)                                                      | **141.23 kB** | **+~16 kB** | The hero now animates with framer on **first paint**, so `domAnimation` is injected **eagerly** at the app root (was a lazy chunk) — ~14 kB gz folds into the entry. Everything else is below-the-fold/lazy (savings hero cards, cycle + trend + 2-up showcases all ride lazy chunks, ~0 entry). Entry decomposition (gz): **framer ~38 kB (27 %)**, router ~32, react-dom ~24, query ~13, radix ~5.5, **TopNav only ~2.7 kB** (its menus already split in Batch 19; showcases already lazy — so neither was a worthwhile lever). JS ceiling **135 → 150**; the one real lever (eager-framer) is left for **B8/T-fe-perf** to weigh holistically rather than patch per-page. CSS 15.27 kB. |

**Bundle-budget posture (post-landing-refresh).** Initial JS **141.23 kB gz**
(budget **150 kB**, ~8.8 kB headroom). The growth since deploy is deliberate:
the **eager `domAnimation`** that lets the landing hero animate with framer on
first paint (~38 kB of the entry is framer; the rest is unavoidable
react/router/query core ~70 kB + app shell). Initial CSS **15.27 kB gz**
(budget 16 kB). Earlier deploy posture was 111.63 kB / 125 kB at `a963f87`
(2026-06-06).
