...
function TagRow({ t, onEdit, onDelete, constants, level }) {
  ...
  const isSystem = t.created_by === null || t.created_by === constants?.SYSTEM_USER_ID;
  const isRestricted = 
  ...
  return (
    <>
      <li style={{ 
        ...
        <div style={{ paddingLeft: `${level * 1.5}rem`, flex: 1, display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
          <button
          ...
          <span style={{ color: '#666', fontSize: '0.8em', whiteSpace: 'nowrap' }}>[{t.tag_type}]</span>
          {t.aliases && t.aliases.length > 0 && (
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px', marginLeft: '4px' }}>
              {t.aliases.map((a, i) => (
                <span key={i} style={{ fontSize: '0.7rem', padding: '2px 8px', borderRadius: '10px', background: '#f1f5f9', color: '#64748b', fontWeight: 600 }}>
                  {a}
                </span>
              ))}
            </div>
...
export function CategoriesTab() {
  ...
  const [form, setForm] = useState({ tag_name: '', parent: '', tag_type: 'discretionary', aliases: [] });
  ...
  const resetForm = () => {
    setForm({ tag_name: '', parent: '', tag_type: 'discretionary', aliases: [] });
    setAliasTemp('');
  ...
  const isSystemTag = editingTagId && (() => {
    const tag = flatTags.find(t => t.tag_id === editingTagId);
    return tag?.created_by === null || tag?.created_by === constants?.SYSTEM_USER_ID;
  })();
  ...
  return (
    <div>
      ...
      <p style={{ color: '#666', marginBottom: '1.5rem', lineHeight: '1.4' }}>
        System tags (seeded defaults) can only be partially edited (you can add aliases and change tag types).
        <br />
        You can add your own custom categories or subcategories below.
        <br />
        <strong>Aliases help the auto-categorization engine to tag your transactions correctly.</strong>
      </p>
        ...
        >
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1.25rem' }}>
            <label style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              <span style={{ fontSize: '0.85rem', fontWeight: 600, color: '#475569' }}>Tag name</span>
              <input
                value={form.tag_name}
            ...
            <label style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              <span style={{ fontSize: '0.85rem', fontWeight: 600, color: '#475569' }}>Tag Type</span>
              <select
                value={form.tag_type}
            ...
            <label style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              <span style={{ fontSize: '0.85rem', fontWeight: 600, color: '#475569' }}>Parent (optional)</span>
              <select
                value={form.parent}
                disabled={isSystemTag}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
            <span style={{ fontSize: '0.85rem', fontWeight: 600, color: '#475569' }}>Aliases</span>
            <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '0.5rem' }}>
              ...
              <button 
                ...
                style={{ padding: '0.6rem 1.2rem', background: '#f1f5f9', border: '1px solid #e2e8f0', borderRadius: '8px', fontWeight: 600, cursor: 'pointer' }}
              >
                Add
              </button>
            </div>
            <div style={{ padding: '0.75rem', background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '8px', minHeight: '3rem', display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
              {form.aliases.length === 0 ? (
                <span style={{ color: '#94a3b8', fontSize: '0.85rem' }}>No aliases added</span>
              ) : form.aliases.map(a => (
                <span key={a} style={{ display: 'flex', alignItems: 'center', gap: '4px', background: '#eff6ff', color: '#2563eb', padding: '4px 10px', borderRadius: '20px', fontSize: '0.85rem', fontWeight: 600, border: '1px solid #dbeafe' }}>
                  {a}
                  <button type="button" onClick={() => handleRemoveAlias(a)} style={{ background: 'none', border: 'none', color: '#2563eb', cursor: 'pointer', padding: 0, fontWeight: 700, fontSize: '1rem' }}>×</button>
                </span>
              ))}
            </div>
            <input readOnly value={form.aliases.join(', ')} style={{ marginTop: '0.5rem', padding: '0.5rem', background: '#f3f4f6', border: '1px solid #e2e8f0', color: '#64748b', fontSize: '0.85rem', borderRadius: '4px' }} />
          </div>
      ...
      {error && <div style={{ color: '#ef4444', marginBottom: '1rem', fontWeight: 600, background: '#fef2f2', padding: '0.75rem', borderRadius: '8px' }}>{error}</div>}

      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
        <h3 style={{ margin: 0 }}>All tags</h3>
        <span style={{ color: '#666', fontSize: '0.9rem' }}>{flatTags.length} tags total</span>
      </div>
      
      <div style={{ border: '1px solid #e2e8f0', borderRadius: '12px', overflow: 'hidden', background: 'white', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
        {loading ? (
          <p style={{ padding: '2rem', textAlign: 'center', color: '#94a3b8' }}>Loading tags...</p>
        ) : (
          <TagTree 
            tags={tags} 
            onEdit={handleEdit} 
