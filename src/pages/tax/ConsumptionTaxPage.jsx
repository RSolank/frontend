...
export function ConsumptionTaxPage() {
  ...
  const canGenerateForRange = (ps, pe) => {
    ...
    if (endDate >= precedingWeekStart)
      return {
        ok: false,
        reason: 'Can only generate for periods prior to the preceding week',
      };
    return { ok: true };
  };
  ...
  const handlePay = async (billId) => {
    setError(null);
    try {
      await apiFetch(`/api/consumption-tax/bills/${billId}/pay`, {
        method: 'POST',
      });
      await loadBills();
      setSelectedBill(null);
      ...
      >
        <div>
          ...
          <p style={{ color: '#64748b' }}>
            Generate and track your taxation insights
          </p>
        </div>
        <Link
          to="/dashboard"
      ...
      >
        ...
        >
          ...
          <div style={{ color: '#666', fontSize: '0.9rem' }}>
            Allowed only for weeks prior to the preceding week (ending before{' '}
            {toISODate(precedingWeekStart)}).
          </div>
        </div>
          ...
          <div style={{ display: 'grid', gap: '0.75rem' }}>
            {bills.map((b) => (
              ...
              >
                ...
                selectedBill &&
                selectedBill.bill_id === b.bill_id ? (
                  <div
                    style={{
                      marginTop: '0.75rem',
                  ...
                  >
                    ...
                    >
                      <div>
                        <b>Tax total</b>: {user?.currency || '$'}
                        {Number(selectedBill.totals?.tax_total || 0).toFixed(2)}
                      </div>
                      <div>
                        <b>Penalty total</b>: {user?.currency || '$'}
                        {Number(
                          selectedBill.totals?.penalty_total || 0
                        ).toFixed(2)}
                      </div>
                    </div>
