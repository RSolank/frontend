...
export function EditTransactionPage() {
  ...
  const availableTags = tags.filter((t) => {
    ...
    const hasOtherTags = form?.tag_ids?.some(
      (id) => id !== constants?.MISCELLANEOUS_TAG_ID
    );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const tagsChanged =
        JSON.stringify(form.tag_ids.sort()) !==
        JSON.stringify(txn.tag_ids.sort());
      ...
      if (tagsChanged && form.beneficiary_id) {
        ...
        if (createRule) {
          ...
          const existingRule = rules.find(
            (r) => r.beneficiary_id === form.beneficiary_id
          );

          if (existingRule) {
            if (
              window.confirm(
                `A rule for "${existingRule.beneficiary_name}" already exists. Update it with these tags?`
              )
            ) {
      ...
      await apiFetch(
        `/api/transactions/${id}${ruleIdToLink ? `?rule_id=${ruleIdToLink}` : ''}`,
        {
          method: 'PATCH',
                  ...
                  {availableBeneficiaries.map((b, i) => (
                    <div
                      ...
                      style={{
                        padding: '0.6rem 1rem',
                        cursor: 'pointer',
                        background: activeBIndex === i ? '#f1f5f9' : 'white',
                      }}
                    >
                      {b.name}
                    </div>
