...
const mockList = [mockBeneficiary, { uid: 6, name: 'Other', aliases: [], beneficiary_type: 'merchant' }];

const renderDetail = () =>
  render(
    <MemoryRouter initialEntries={['/beneficiaries/5']} future={{ v7_startTransition: true, v7_relativeSplatPath: true }}>
      <Routes>
...
describe('BeneficiaryDetailPage', () => {
  ...
  it('renders read-only beneficiary with alias chips', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByRole('heading', { name: 'Mobile Recharged' })).toBeInTheDocument();
    });
  ...
  it('shows merge form in edit mode and checks alias uniqueness', async () => {
    apiFetch.mockImplementation(async (url, options) => {
      if (url === '/api/beneficiaries/5') return mockBeneficiary;
      if (url === '/api/beneficiaries') return mockList;
      if (url.includes('/check-alias')) return { alias: 'NewAlias', unique: true };
    ...
    await waitFor(() => {
      expect(apiFetch).toHaveBeenCalledWith('/api/beneficiaries/5', expect.objectContaining({
        method: 'PATCH',
  ...
  it('switches form fields when the beneficiary type changes', async () => {
    apiFetch.mockImplementation(async (url) => {
      if (url === '/api/beneficiaries/5') {
        return {
          ...mockBeneficiary,
          merchant: { category: 'utility', contact: '9999999999', upi_id: 'store@upi' },
        };
      }
      if (url === '/api/beneficiaries') return mockList;
      return {};
  ...
  it('warns when merge source and target types differ', async () => {
    apiFetch.mockImplementation(async (url) => {
      ...
      if (url === '/api/beneficiaries') {
        return [
          mockBeneficiary,
          { uid: 6, name: 'Jane', aliases: [], beneficiary_type: 'person', person: { relationship_type: 'friend' } },
        ];
      }
      return {};
    });
    ...
    fireEvent.change(screen.getByLabelText('Into Target (will keep)'), { target: { value: '6' } });

    await waitFor(() => {
      expect(screen.getByText(/Type mismatch detected/i)).toBeInTheDocument();
      expect(screen.getByText(/merchant fields map to merchant fields/i)).toBeInTheDocument();
    });
  });
});
