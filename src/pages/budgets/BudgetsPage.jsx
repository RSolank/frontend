...
export function BudgetsPage() {
  ...
  const loadStatus = async (monthOverride = null) => {
    ...
    setError(null);
    try {
      const url = monthOverride
        ? `/api/budget-limits/status?month=${monthOverride}`
        : '/api/budget-limits/status';
      const res = await apiFetch(url);

      // Filter out categories with zero expense for the selected month
      const activeCategories = (res.categories || []).filter(
        (cat) => (cat.current_expense || 0) > 0
      );
  ...
  const handleStartEdit = (cat) => {
    ...
    setEditLimit(
      cat.limit_amt !== null
        ? cat.limit_amt
        : ((cat.avg_expense || 0) * 1.2).toFixed(2)
    );
    setEditPenalty(
      (cat.penalty_rate || cat.default_penalty_rate || 0.05).toString()
    );
  };
        ...
        <div
          style={{
            ...
            background: isOver
              ...
              : 'linear-gradient(90deg, #4d94ff, #0066ff)',
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <div>
            <h3 style={{ margin: 0, fontSize: '1.1rem', color: '#004a99' }}>
            ...
            >
              Overall soft-limit for the month
            </p>
          </div>
          <div style={{ textAlign: 'right' }}>
            ...
            <div
              style={{ fontSize: '1.5rem', fontWeight: 700, color: '#0052cc' }}
            >
              {currencySymbol}
              {(totalBudget?.current_expense || 0).toLocaleString()}
              {totalBudget?.limit_amt ? (
                <span
                  style={{
                    fontSize: '0.9rem',
                    color: '#a0aec0',
                ...
                >
                  / {currencySymbol}
                  {totalBudget.limit_amt.toLocaleString()}
                </span>
              ) : null}
            </div>
          </div>
          ...
          >
            ...
            <div style={{ display: 'flex', gap: '1rem', textAlign: 'right' }}>
              <div>
                ...
                <div style={{ fontWeight: 600, color: '#004a99' }}>
                  {currencySymbol}
                  {(totalBudget?.avg_expense || 0).toFixed(0)}
                </div>
              </div>
              <button
                onClick={() =>
                  handleStartEdit({
                    ...totalBudget,
                    tag_id: constants?.TOTAL_TAG_ID || 1,
      ...
      {expandedTagId === (constants?.TOTAL_TAG_ID || 1) && (
        ...
        >
          <div style={{ marginBottom: '1.5rem' }}>
            ...
            >
              <label style={{ fontWeight: 600, color: '#004a99' }}>
                Global Monthly Limit
              </label>
              <span style={{ color: '#0066ff', fontWeight: 700 }}>
                {currencySymbol}
                {Number(editLimit).toLocaleString()}
              </span>
            </div>
            <input
              type="range"
      ...
      >
        {categories.map((cat) => {
          ...
          const statusColor =
            cat.limit_amt && cat.current_expense > cat.limit_amt
              ? '#e53e3e'
              : '#2d3748';
                ...
                <div style={{ flex: 1 }}>
                  ...
                  >
                    <div>
                      ...
                      >
                        {currencySymbol}
                        {(cat.current_expense || 0).toLocaleString()}
                        {cat.limit_amt ? (
                          <span
                            style={{
                              fontSize: '0.9rem',
                    ...
                    >
                      ...
                      <div>
                        ...
                        <div style={{ fontWeight: 600 }}>
                          {currencySymbol}
                          {(cat.min_expense || 0).toFixed(0)} - {currencySymbol}
                          {(cat.max_expense || 0).toFixed(0)}
                        </div>
                      </div>
                    </div>
                  </div>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      >
                        ...
                        <span style={{ fontSize: '0.85rem', color: '#666' }}>
                          (~ {((parseFloat(editPenalty) || 0) * 100).toFixed(0)}
                          % surcharge on tax)
                        </span>
                      </div>
                      <p
                      ...
                      >
                        Default for {cat.tag_type} is{' '}
                        {((cat.default_penalty_rate || 0.05) * 100).toFixed(0)}
                        %.
                      </p>
                    </div>
