...
export function DashboardPage() {
  ...
  useEffect(() => {
    // Load data
    apiFetch('/api/budget-limits/status')
      .then(setBudgetData)
      .catch(() => {});
    apiFetch('/api/consumption-tax/bills')
      .then((d) => {
        const currentMonth = new Date().toISOString().slice(0, 7);
        const total = (d.bills || [])
          .filter((b) => b.period_start?.startsWith(currentMonth))
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
          ...
          >
            <div style={{ position: 'relative', zIndex: 1 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <span
                  style={{ fontSize: '0.85rem', fontWeight: 600, opacity: 0.9 }}
                >
                  TOTAL BUDGET
                </span>
                {taxSummary > 0 && (
              ...
              >
                {currency}
                {(
                  budgetData?.total_budget?.current_expense || 0
                ).toLocaleString()}
              </h2>
              <div style={{ opacity: 0.8, fontSize: '0.95rem' }}>
                spent of {currency}
                {(
                  budgetData?.total_budget?.limit_amt || 0
                ).toLocaleString()}{' '}
                limit
              </div>
              <div style={{ marginTop: '1.5rem' }}>
                <ProgressBar
                  current={budgetData?.total_budget?.current_expense || 0}
                  limit={budgetData?.total_budget?.limit_amt}
                  color="#ffffff"
                />
              </div>
              <div style={{ marginTop: '1.5rem' }}>
          ...
          >
            ...
            <Link to="/upload-statement" style={{ textDecoration: 'none' }}>
              <Card
                style={{
                  padding: '1.25rem',
                  textAlign: 'center',
