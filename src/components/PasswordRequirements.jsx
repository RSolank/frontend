...
export function PasswordRequirements({ password }) {
  ...
  const { length, uppercase, lowercase, digit, special } = validatePassword(password);

  const Requirement = ({ met, text }) => (
    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.85rem', color: met ? '#059669' : '#6b7280', transition: 'color 0.2s' }}>
  ...
  return (
    <div style={{ 
      ...
      <p style={{ margin: '0 0 0.25rem 0', fontSize: '0.8rem', fontWeight: 'bold', color: '#374151' }}>Password must have:</p>
