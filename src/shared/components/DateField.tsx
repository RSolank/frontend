...
export function DateField({
  ...
  // viewMonth is `YYYY-MM` — anchored to the value when valid, else
  // today. Resets every time the popup opens so an old session
  // doesn't strand the user on an irrelevant month.
  ...
  // Reset the view month each time the modal reopens.
  useEffect(() => {
    if (!open) return;
    setViewMonth(isValidIso(value) ? value.slice(0, 7) : today.slice(0, 7));
  }, [open, value, today]);
          ...
          {grid.map((cell) => {
            ...
            return (
              <button
                ...
                className={`flex h-9 items-center justify-center rounded-md text-sm tabular-nums transition-colors focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:outline-none ${
                  isSelected
                    ? 'bg-indigo-600 font-semibold text-white hover:bg-indigo-700'
                    : isToday
                      ? 'bg-indigo-50 font-semibold text-indigo-700 hover:bg-indigo-100 dark:bg-indigo-950/40 dark:text-indigo-200 dark:hover:bg-indigo-950/60'
                      : cell.inMonth
