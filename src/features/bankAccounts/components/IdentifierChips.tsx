...
interface Props {
  ...
  identifiers: ReadonlyArray<
    AccountIdentifier | AccountIdentifierCreatePayload
  >;
  onRemove: (
...
export function IdentifierChips({
  identifiers,
  onRemove,
  busy = false,
}: Props) {
  if (identifiers.length === 0) {
    return (
      ...
      >
        No identifiers yet. UPI handles let statement uploads auto-attribute
  ...
  return (
    ...
    >
      {identifiers.map((identifier, index) => (
        <li
          key={
            'uid' in identifier ? `live-${identifier.uid}` : `pending-${index}`
          }
        >
          <span className="inline-flex items-center gap-1.5 rounded-full border border-slate-300 bg-slate-50 px-2 py-1 text-xs dark:border-slate-700 dark:bg-slate-800">
            ...
            <button
              ...
              className="hover:bg-danger-100 hover:text-danger-700 focus-visible:ring-danger-500 dark:hover:bg-danger-950/50 dark:hover:text-danger-300 inline-flex h-4 w-4 items-center justify-center rounded-full text-slate-500 transition-colors focus-visible:ring-2 focus-visible:outline-none disabled:cursor-not-allowed disabled:opacity-50 dark:text-slate-400"
            >
              <X size={10} />
            </button>
          </span>
