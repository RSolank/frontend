...
// `BudgetSignal` pill and the `SpendGauge` fill, so the page never shows the
...
//   • no limit   → position vs the rolling "typical" baseline the BE now stores
...
// danger / slate) — the same palette the old StatusBar + AnomalyBadge used.
