...
describe('AccountAccessibilityPage', () => {
  it('renders the seven Display & motion controls', () => {
    ...
    // Zoom — slider.
    expect(screen.getByRole('slider', { name: /Text size/i })).toBeInTheDocument();
    ...
    expect(screen.getByRole('switch', { name: /hide amounts/i })).toBeInTheDocument();
    expect(screen.getByRole('switch', { name: /high contrast/i })).toBeInTheDocument();
    expect(screen.getByRole('switch', { name: /underline links/i })).toBeInTheDocument();
    expect(screen.getByRole('switch', { name: /always show focus/i })).toBeInTheDocument();
  });
